    ...
    protected void randomTick(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        if (blockState.getValue(LIT)) {
            ...
            for (Player player : list) {
                player.addEffect(new MobEffectInstance(MobEffects.STRENGTH, 200 * blockState.getValue(CANDLES), 0, true, true));
                player.addEffect(new MobEffectInstance(MobEffects.SPEED, 200 * blockState.getValue(CANDLES), 0, true, true));
            }
        }
    }
